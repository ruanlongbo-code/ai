...
prompt = PromptTemplate(
    ...
    template="""
           ...
           - Workflow:
             1. 仔细阅读需求文档（包含原始需求和知识增强信息），理解功能描述和业务逻辑。
             2. 如果文档中包含"RAG知识库补充信息"，请结合该部分内容理解完整的业务背景和技术约束。
             3. 如果文档中包含"评审会议知识"，请特别关注评审中提到的关键决策、遗漏场景和补充说明，
                这些往往是需求文档中没有体现但必须测试的场景。
             4. 如果文档中包含"历史用例参考"，请参考已有用例避免遗漏，同时避免重复。
             5. 根据功能描述，列出功能正常情况下的测试点。
             6. 分析功能的边界条件，设计边界测试点。
             7. 考虑可能出现的异常情况，设计异常处理测试点。
             8. 结合评审知识中的关键决策和待办事项，补充额外的测试点。
