    ...
    try:
        ...
        async def generate_stream():
            ...
            try:
                ...
                yield f"data: {json.dumps({'type': 'info', 'message': f'【知识增强】已检索到 {len(knowledge_sources)} 个知识源：{source_names_str}'}, ensure_ascii=False)}\n\n"
                ...
                # 发送开始生成的消息
                yield f"data: {json.dumps({'type': 'start', 'message': '【开始生成】基于增强知识调用AI大模型...'}, ensure_ascii=False)}\n\n"
                ...
                # 发送完成消息
                yield f"data: {json.dumps({'type': 'complete', 'message': f'测试用例生成完成！共生成 {generated_cases_count} 个测试用例（知识源：{source_names_str}）'}, ensure_ascii=False)}\n\n"
